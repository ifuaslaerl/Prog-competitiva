...
int main(){
    ...
    set<char> letras ;
    map<char,int> quantidade ;

    for( int i = 0 ; i < int(palavra.size()) ; i++ ){
        letras.insert(palavra[i]) ;
        quantidade[palavra[i]]++ ; 
    }
    ...
    if( existe ) resp += impar ;

    reverse(metade.begin(),metade.end()) ;
