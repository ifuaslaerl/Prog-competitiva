...
int main(){
    ...
    vector<pair<int,int>> eventos(2*n) ; // registra respectivamente ( horario , tipo_de_evento )

    for ( int i = 0 ; i < 2*n ; i++ ){
        cin >> eventos[i].first ;
        eventos[i].second = i%2 ; // 0 = entrada ; 1 = saida
    }

    sort(eventos.begin(),eventos.end()) ; // organiza os eventos por horario

    int cont = 0 , resp = 0 ;
    for( int i = 0 ; i < 2*n ; i++ ){
        
        pair<int,int> atual = eventos[i] ;

        if ( !atual.second ) cont++ ;
        else cont-- ;
