...
int main(){
    ...
    for( int i = 0 ; i < m ; i++ ) cin >> compradores[i] ;

    for( int i = 0 ; i < m ; i++){
        auto it = tickets.upper_bound(compradores[i]) ;
        it-- ;
        
        if ( *it > compradores[i] || tickets.find(*it) == tickets.end() ) cout << -1 << "\n" ;
        else{
            cout << *it << "\n" ;
            tickets.erase(it) ;
        }
        ...
        // cout << "\n" ;
        // for ( int elemento : tickets ) cout << elemento << " " ;
        // cout << "\n\n" ;
    }
